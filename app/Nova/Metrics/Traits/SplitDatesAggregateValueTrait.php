...
{
    public function aggregateSplit(
        $request,
        $queryOrModel,
        $function,
        $column = null,
        ...
        $previousValue = round(with(clone $query)
            ->whereBetween($dateColumn,
                [$previousRange[0]->format($dateFormat), $previousRange[1]->format($dateFormat)])
        ...
        return $this->result(
            round(with(clone $query)
                ->whereBetween($dateColumn,
                    [$currentRange[0]->format($dateFormat), $currentRange[1]->format($dateFormat)])
                ->{$function}($column), $this->precision)
